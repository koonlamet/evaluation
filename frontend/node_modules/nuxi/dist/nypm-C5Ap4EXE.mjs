...
import { t as __exportAll$1 } from "./rolldown-runtime-CGrdTa9n.mjs";
...
//#region ../../node_modules/.pnpm/giget@3.3.0/node_modules/giget/dist/_chunks/libs/nypm.mjs
...
function j(e, t) {
	...
	let o = r.concat(n).join(delimiter);
	return {
		key: t.key,
		value: o
	};
...
const N = (e) => {
	...
	for (let t of e) pipeline$1(t, n, { end: !1 }).then(r).catch(r);
	return n;
...
function B(e, n = [], a = {}) {
	...
	if (o !== null) {
		...
		try {
			n = openSync(o, `r`), readSync(n, e, 0, 150, 0);
		} catch {} finally {
			n !== null && closeSync(n);
		}
		let a = e.toString().match(F);
		if (a !== null) {
			let e = a[1].trim(), t = e.indexOf(` `), n = t === -1 ? e : e.slice(0, t), r = t === -1 ? `` : e.slice(t + 1), i = basename(n);
			s = i === `env` ? r || null : i;
		}
	}
	if (s !== null && o !== null && (n = [o, ...n], e = s, o = V(e, a)), o === null || !I.test(o)) {
		let t = o !== null && L.test(o);
		e = normalize(e), e = e.replace(P, `^$1`), n = n.map((e) => (e = e.replace(/(?=(\\+?)?)\1"/g, `$1$1\\"`), e = e.replace(/(?=(\\+?)?)\1$/, `$1$1`), e = `"${e}"`, e = e.replace(P, `^$1`), t && (e = e.replace(P, `^$1`)), e)), n = [
			`/d`,
			`/s`,
			`/c`,
			`"${[e, ...n].join(` `)}"`
...
function V(e, t) {
	let n = (t.cwd ?? cwd()).toString(), r = t.env ?? process.env, i = A(r).value, o = e.includes(`/`) || e.includes(`\\`) ? [``] : [n, ...i.split(delimiter)], s = r.PATHEXT ? r.PATHEXT.split(delimiter) : z;
	e.includes(`.`) && s[0] !== `` && s.unshift(``);
	for (let t of o) {
		let r = resolve(n, t.startsWith(`"`) && t.endsWith(`"`) && t.length > 1 ? t.slice(1, -1) : t, e);
		for (let e of s) {
			let t = r + e;
			try {
...
var q = class {
	...
	async *[Symbol.asyncIterator]() {
		...
		let n = N(t), r = u.createInterface({ input: n });
	...
	spawn() {
		...
		let i = B(this._command, this._args, n), a = spawn(i.command, i.args, i.options);
...
async function $(e, t = {}) {
	let r = await ie(C(e || `.`), async (e) => {
		if (!t.ignorePackageJSON) {
			...
			if (existsSync(x(e, `deno.json`))) return Q.find((e) => e.name === `deno`);
		}
		if (!t.ignoreLockFile) {
			for (let t of Q) if ([t.lockFile, t.files].flat().filter(Boolean).some((t) => existsSync(C(e, t)))) return { ...t };
		}
...
//#endregion
export { re as a, nypm_exports as i, D as n, E as r, C as t };
